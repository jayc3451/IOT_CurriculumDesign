...
import { temp_1, humi_1 } from '@/api/getDevProp';
...
// 由于temp_1和humi_1是同步更新的，我们只需要监听其中一个即可
watch(temp_1, (newData) => {
  if (newData && humi_1.value) {
    addData({
      time: newData.time,
      value: newData.value
    }, {
      time: humi_1.value.time,
      value: humi_1.value.value
    });
  }
});
